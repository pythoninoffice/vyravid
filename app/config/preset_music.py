"""
Preset Music Configuration for Video Generation

This module defines the available background music tracks that users can select.
All music files are stored locally under data/storage/music and served at /media/music.
"""
...
# Preset music library - URLs point to local media storage.
...
def get_music_by_category(category: MusicCategory) -> List[Dict[str, Any]]:
    """Get all music tracks in a specific category"""
    return [track for track in PRESET_MUSIC_LIBRARY if track["category"] == category]

def get_music_by_mood(mood: MusicMood) -> List[Dict[str, Any]]:
    """Get all music tracks with a specific mood"""
    return [track for track in PRESET_MUSIC_LIBRARY if track["mood"] == mood]

def get_music_by_id(music_id: str) -> Dict[str, Any]:
...
def search_music(query: str, category: str = None, mood: str = None) -> List[Dict[str, Any]]:
    """Search music tracks by name, description, or tags"""
    results = PRESET_MUSIC_LIBRARY.copy()
    
    # Filter by category
    if category:
        results = [track for track in results if track["category"] == category]
    
    # Filter by mood
    if mood:
        results = [track for track in results if track["mood"] == mood]
